...
parser.add_argument('--model_card', default='gpt2.sm', choices=['gpt2.sm', 'gpt2.md', 'gpt2.lg','gpt2.smallGPU'],
                    help='model names')
...
def beam(model, data_iter, args):
    ...
    with torch.no_grad():
        for idx, data in enumerate(data_iter):
            ...
            # FIX: Create correct row indices for batch_size > 1
            _row_indices = torch.arange(batch_size * num_beams, device=_query.device)
            ...
            with torch.no_grad():
                for i in range(0, args.eval_len):
                    if i == 0:
                        ...
                        # FIX: Use _row_indices instead of _bbatch for correct indexing
                        logits = logits[_row_indices, (_query_len-1).long(), :] 
                    else:
...
if __name__ == '__main__':
    ...
    lm_net = lm_net.to('cuda:'+str(device_number))#.cuda()
