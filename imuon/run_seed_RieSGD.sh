#!/bin/bash
# Variance / multi-seed runner for Riemannion (SGD, no momentum) on E2E NLG.
# Sister script to run_seed_experiment.sh, but uses a different training
# driver and a different LoRA parameterization (the Riemannion fork uses
# 2r-block parameterization, so lora_dim is doubled at beam-decode time).
#
# Required env var:
...
#   WORK_DIR   override training output dir (default: $IMUON_ROOT/runs/riemannion_sgd_nomom_seed<SEED>)
#   HF_HOME    HuggingFace cache            (default: $IMUON_ROOT/.cache/hf)
#
# Best-LR pinning (from Table 5, no-momentum block):
#   Riemannion (SGD) → lr=5e-2  (BLEU 70.02 at seed=110)
#
# Usage:
...
if [ -z "${SEED:-}" ]; then
    echo "ERROR: SEED env var must be set (e.g. SEED=42 bash $0)" >&2
    exit 1
fi
...
WORK_DIR="${WORK_DIR:-$IMUON_ROOT/runs/riemannion_sgd_nomom_seed${SEED}}"
...
DRIVER="$IMUON_ROOT/examples/riemannian_muon_lora/pilancilab_pipeline/gpt2_ft_riemannion_sgd.py"

LR=0.05
EXP_NAME="riemannion_sgd_nomom_seed${SEED}"
EFFECTIVE_RANK=4
MODEL_LORA_DIM=8   # Riemannion's 2r-block parameterization doubles the rank at beam time.
LORA_ALPHA=64      # alpha=64 with model_lora_dim=8 → scaling 64/8=8, matching pilancilab's 32/4=8 for r=4.
TARGET_BLEU="70.02"
...
if [ ! -f "$DRIVER" ]; then
    echo "ERROR: Riemannion driver not found at $DRIVER" >&2
    exit 1
fi
...
echo "============================================================"
echo "Method: Riemannion (SGD, no momentum)  | Seed: $SEED  | LR: $LR"
echo "Effective rank: $EFFECTIVE_RANK  | Model lora_dim (beam): $MODEL_LORA_DIM  | alpha: $LORA_ALPHA"
...
"$PYTHON" -m torch.distributed.launch --nproc_per_node=1 --master_port="$MASTER_PORT" \
    ...
    --random_seed "$SEED" \
    --trial_name "$EXP_NAME" \
    --riemannian_optimizer sgd \
    --riemannian_sgd_momentum 0.0

echo ""
echo "Training complete: $(date)"

# ---- 2. Beam decode ----
# Driver writes model.<trial>.{step,best,final}.pt — different filename pattern from gpt2_ft_muon.
FINAL_CKPT="$(ls -t "$WORK_DIR"/model.${EXP_NAME}.final.pt \
                    "$WORK_DIR"/model.${EXP_NAME}.best.pt \
                    "$WORK_DIR"/model.${EXP_NAME}.*.pt 2>/dev/null | head -1 || true)"
...
echo ""
echo "============================================================"
echo "RESULTS — Riemannion (SGD) | seed=$SEED | lr=$LR"
echo "============================================================"
